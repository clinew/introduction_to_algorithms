...
void selection_sort(void* array, size_t item_count, size_t item_size, int(*comparator)(void*, void*)) {
	int i;
	int j;
	int index;
	void* temp;
	...
	for (i = 0; i < item_count; i++) {
		// Search for the smallest element in the specified array.
		index = i;
		for (j = (i + 1); j < item_count; j++) {
			if (comparator((void*)(((char*)array) + (index * item_size)),
				       (void*)(((char*)array) + (j * item_size))) > 0) {
				index = j;
			}
		}

		// Swap the items between index and i;
		if (i == index) {
			continue;
		}
		memcpy(temp, (void*)(((char*)array) + (i * item_size)), item_size);
		memcpy((void*)(((char*)array) + (i * item_size)),
		       (void*)(((char*)array) + (index * item_size)), item_size);
		memcpy((void*)(((char*)array) + (index * item_size)), temp, item_size);
	}
...
int main(int argc, char* argv[]) {
	...
	printf("\n");
	selection_sort(ints, sizeof(ints)/sizeof(int), sizeof(int), int_comparator);
	for (i = 0; i < (sizeof(ints)/sizeof(int)); i++) {
		printf("%i ", ints[i]);
	}
	printf("\n");
	...
	printf("\n");
	selection_sort(doges, sizeof(doges)/sizeof(struct doge), sizeof(struct doge), doge_comparator);
	for (i = 0; i < (sizeof(doges)/sizeof(struct doge)); i++) {
		doge_print(&doges[i]);
	}
	printf("\n");
